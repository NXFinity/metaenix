...
export const Comments = ({ postId, allowComments = true, onCommentAdded, limit = 50 }: CommentsProps) => {
  ...
  // Comments query
  // Public posts have public comments, private post comments are only accessible to post owner
  const { data: commentsData, isLoading: isLoadingComments, refetch: refetchComments } = useQuery({
    ...
    retry: (failureCount, error: unknown) => {
      // Don't retry on 403 (Forbidden) or 404 (Not Found) errors, or connection errors
  ...
  const createCommentMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['postAnalytics', postId] });
