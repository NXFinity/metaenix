...
const ReplyItem = ({ reply, photoId, isAuthenticated, currentUserId, onLike, onView, onDelete, likePending, deletePending }: ReplyItemProps) => {
  return (
...
export const PhotoComments = ({ photoId, allowComments = true, onCommentAdded, limit = 50 }: PhotoCommentsProps) => {
  ...
  const { data: commentsData, isLoading: isLoadingComments, refetch: refetchComments } = useQuery({
    ...
    queryFn: () => photosService.getComments(photoId, { page: 1, limit }),
    enabled: allowComments !== false && !!photoId,
  ...
  const likeCommentMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['photos', photoId, 'comments'] });
    },
  });
  ...
  const createCommentMutation = useMutation({
    mutationFn: (content: string) =>
      photosService.createComment(photoId, {
        content,
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['photoAnalytics', photoId] });
  ...
  const deleteCommentMutation = useMutation({
    mutationFn: (commentId: string) => photosService.deleteComment(commentId),
    onSuccess: () => {
  ...
  const handleReplyClick = (comment: Comment) => {
    ...
    setTimeout(() => {
      document.getElementById(`comment-input-${photoId}`)?.focus();
    }, 100);
  };
  ...
  const handleViewComment = (comment: Comment) => {
    if (comment.user?.username) {
      router.push(`/${comment.user.username}/posts/comment/${comment.id}?photoId=${photoId}`);
    }
  };
