...
const postsService = {
  ...
  async like(postId: string): Promise<LikeResponse> {
    return likesService.like('post', postId);
  },
  ...
  async unlike(postId: string): Promise<LikeResponse> {
    return likesService.unlike('post', postId);
  },
  ...
  async share(
    ...
    return sharesService.share('post', postId, data);
  },
  ...
  async getComments(
    ...
    return commentsService.getForResource('post', postId, params);
  },
  ...
  async createComment(
    ...
    return commentsService.create('post', postId, data);
  },
   ...
   * @deprecated Replies are now included in getCommentById. Use that instead.
   */
  async getCommentReplies(
    commentId: string,
    params?: PaginationParams,
  ): Promise<PaginationResponse<Comment>> {
    // Replies are included in the comment response from getById
    ...
    return {
      ...
      meta: {
        ...
        total: comment.repliesCount || 0,
        totalPages: Math.ceil((comment.repliesCount || 0) / (params?.limit || 20)),
        hasNextPage: false,
  ...
  async getCollections(
    ...
    const response = await apiClient.get<PaginationResponse<Collection>>(
      POSTS_ENDPOINTS.GET_COLLECTIONS,
      { params },
    );
    return response.data;
  },
  ...
  async updateCollection(
    ...
    const response = await apiClient.patch<CollectionResponse>(
      POSTS_ENDPOINTS.UPDATE_COLLECTION(collectionId),
      data,
    );
    return response.data;
  },
